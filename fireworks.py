...
class Projectile:
    ...
    def move(self):
        self.x += self.x_vel
        self.y += self.y_vel
        self.aplha = max(0, self.aplha - self.ALPHA_DECREMENT)

    def draw(self, win):
        self.draw_rect_aplha(win, self.color + (self.aplha,), (self.x, self.y, self.WIDTH, self.HEIGHT))

    @staticmethod
    def draw_rect_aplha(surface, color, rect):
